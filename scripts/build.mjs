...
//   dist/wonderloom-standalone.html   one self-contained file (styles, scripts, portraits inlined)
...
mkdirSync(dist);
for (const item of ['index.html', 'styles', 'src', 'portraits', 'LICENSE']) {
...
for (const path of scripts) {
  ...
  if (path === 'src/core/wonderloom.js') {
    block += '\n' + inlineScript(`Wonderloom.portraitSources = ${JSON.stringify(portraits)};\n`, 'portraits');
  }
...
writeFileSync(join(dist, 'wonderloom-standalone.html'), standalone);
...
console.log(`Built dist/wonderloom-standalone.html (${kb(standalone)})`);
