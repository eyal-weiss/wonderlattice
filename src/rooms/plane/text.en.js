...
<p>Zoom in near a point z and a smooth complex function looks like multiplying by one number, its derivative f′(z): f(z + h) ≈ f(z) + f′(z)·h for a tiny h. So every tiny arrow at z is stretched by |f′(z)| and turned by the angle of f′(z), the same for every direction. The compass's two arrows turn together, and still meet at a right angle. A map that keeps angles like this is called <em>conformal</em>. The grid's lines cross at right angles after bending, too, even when the squares become curved.</p>
