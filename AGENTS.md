...
index.html contains inline CSS and two script closures. The first closure implements motion drawing; the second implements shared room navigation, waves/audio, flocking, the projected ribbon, and traffic UI. The traffic mathematics and rendering live in experiments/traffic.js. Mathematician cameos and illustrated portraits live in experiments/guests.js. Both load as regular scripts for offline use and are copied by Vite during build. Search for roomInfo, insights, controls, choose, stepFlock, wavePoint, surface, drawRibbon, and ranges. Navigation and agent tool enums currently assume exactly five rooms: update all of them when adding a room.
