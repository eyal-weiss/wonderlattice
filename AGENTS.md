...
index.html contains inline CSS and two script closures. The first closure implements motion drawing; the second implements shared room navigation, waves/audio, flocking, the projected ribbon, and traffic UI. The traffic mathematics and rendering live in experiments/traffic.js. Mathematician cameos live in experiments/guests.js; the documented portrait images are in portraits/ with their sources and rights in docs/PORTRAITS.md. The optional discovery trail lives in experiments/trail.js and saves only to browser localStorage, with JSON export/import. All three scripts load normally for offline use, and Vite copies the scripts and portraits during build. Search for roomInfo, insights, controls, choose, stepFlock, wavePoint, surface, drawRibbon, and ranges. Navigation and agent tool enums currently assume exactly five rooms: update all of them when adding a room.
